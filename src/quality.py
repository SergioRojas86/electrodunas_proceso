    ...
    if 'Contents' in response:
        for obj in response['Contents']:
            ...
            if any(key.startswith(directory) for directory in files_to_check):
                if key.endswith('.csv'):
                    logger.info('verificando la completitud de datos')
                    completeness_csv(s3_client, logger, cleaning_bucket, key)
                    # verificar estructura columnas
                    logger.info('verificando que se encuentren las columnas requeridas en todos los archivos de los clientes')
                    if not check_csv_columns(s3_client, cleaning_bucket, key, csv_columns):
                        files_with_missing_columns.append(key)                        
                elif key.endswith('.xlsx'):
                    logger.info('verificando que se encuentren las columnas requeridas en el catalogo de sector economico')
